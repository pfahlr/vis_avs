...
std::array<std::uint8_t, 16> finalize(Md5Context& ctx) {
  ...
  for (int i = 0; i < 8; ++i) {
    lengthBytes[i] = static_cast<std::uint8_t>((ctx.bitCount >> (8 * i)) & 0xFFu);
  }
...
std::string computeMd5Hex(const std::uint8_t* data, std::size_t size) {
  ...
  for (std::size_t i = 0; i < digest.size(); ++i) {
    const std::uint8_t byte = digest[i];
    hex[i * 2] = kHex[(byte >> 4) & 0x0F];
    hex[i * 2 + 1] = kHex[byte & 0x0F];
  }
  return hex;
}
